...
### Confirmed connection geometry (client input, revision 1.6)

The coordinate reference is the structural wall face. For U-Channel, the 110 mm dimension terminates at the **outer channel face**. The waterproofing build-up (2 coats × 2 mm) and optional 50 mm Rock Wool are contained within this distance and are not added again. U/Z/Omega screw projection is 40 mm from the outer support face; L-Bracket projection is 50 mm. Stone penetration is 8 mm for 20 mm stone and 14 mm for 30 mm stone; 25 mm remains `RFI_REQUIRED`. Therefore the rear-fixing U example is stone back = 110 + 40 − 8 = 142 mm and finished face = 162 mm for 20 mm stone (30 mm: 136 / 166 mm). Top-pin horizontal stone-back coordinates must be entered separately because vertical penetration cannot be subtracted from a horizontal projection.

Opening closures are sized from the verified architectural return (for example 70 mm) plus the measured distance to the finished stone face. The program does not invent a return or frame depth: missing jamb, head, sill/threshold and corner-return dimensions remain `RFI_REQUIRED`. Corner returns use the same finished-face offset so adjacent stones close consistently.
